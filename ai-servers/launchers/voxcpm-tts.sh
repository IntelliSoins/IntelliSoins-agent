#!/bin/bash
# VoxCPM2 LoRA Michael v6 (MLX 8bit) — Port 8025
...
VOXCPM_MLX_CKPT="${VOXCPM_MLX_CKPT:-$VOXCPM_DIR/checkpoints-merged-michael-v6-mlx-8bit}"
...
# Fallback: checkpoint v6 mergé perdu (~/apple_all/voxcpm effacé) — voir MEMORY/rapport 2026-07-05.
# mlx_audio.server charge les modèles à la demande ; la voix de base VoxCPM2 reste servie
# tant que le checkpoint fine-tuné n'est pas restauré/re-mergé.
if [ ! -d "$VOXCPM_MLX_CKPT" ]; then
  echo "WARN: checkpoint VoxCPM v6 introuvable: $VOXCPM_MLX_CKPT — fallback voix de base VoxCPM2" >&2
fi
