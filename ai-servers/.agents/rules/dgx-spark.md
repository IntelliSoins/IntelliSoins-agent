...
| -------------------- | ---- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| qwen35-4b-finetunev1 | 8000 | `llm`  | **`Qwen/Qwen3.5-4B` fine-tuné `intellisoins-tasks` v1** (vLLM, container `qwen35-4b-finetunev1`, `--served-model-name qwen3.5-4b-finetunev1` + alias legacy `qwen3.5-4b-intellisoins`, LoRA mergé bf16 → rewrap VL 8,8 Go) — renommé 2026-07-11 (ex `spark-vllm-4b`) | Auth `--api-key` (`SPARK_VLLM_API_KEY` .env = SOPS env.prod VPS, archivée Proton Pass) ; **max-model-len 262144** + **gpu-mem-util 0.5** (KV cache ~47 Go / 1.5M tokens, max-num-seqs 1) depuis 2026-07-11 (test max-contexte) ; `--enable-auto-tool-choice --tool-call-parser qwen3_coder` — **parser corrigé 2026-07-12** (`hermes` → `qwen3_coder`, backup `docker-compose.yml.bak-20260712-toolparser`) : le chat template natif Qwen3.5-4B émet le format XML `<tool_call><function=…><parameter=…>` (Format B, cf. rule training-datasets) que `hermes` (JSON) ne parsait pas → **`tool_choice:"auto"` retourne maintenant des `tool_calls` STRUCTURÉS** (validé via LiteLLM Mac) ; **publié sur sparklan 10.0.1.1:8000** (~4 ms) + mesh 10.0.0.5:8000 (~82 ms) depuis 2026-07-11 (Hammerspoon primaire). `parseHermesToolCalls` Hammerspoon reste comme fallback (init.lua:345 lit `message.tool_calls` d'abord) ; le FT émet aussi `[raisonnement] <​/think> [réponse]` (open tag strippé par vLLM, close tag = délimiteur) — `stripReasoning` ne garde que la réponse ; consommé par Hammerspoon (⌥q/⌥⌘O/⌥⌘ç/⌥4), LiteLLM **VPS** (alias `formation-tutor` via alias legacy) **ET LiteLLM Mac** (`qwen3.5-4b-finetunev1`, opencode) |
...
### ⚠️ Incident 2026-07-12/13 — wedge total 13h (task-102) + protections

**Ce qui s'est passé** : FT Qwen3.5-4B **v2** (`run-qwen35-4b-v2.sh`, dataset `dgx-fable5-mix-v2`) lancé 2026-07-12 18:51 → gelé au **step 0/368** (un exemple de **1 003 085 tokens** > max 262144 dans le dataset ; forward bf16 + 6 containers serving résidents = starvation mémoire unifiée). Résultat 13h : toute inférence GPU gelée (whisper/NLLB/embeddings/reranker hang, HTTP statique répondait), **sshd wedged** (banner exchange timeout — accès distant perdu), reboot physique requis. Aucun checkpoint v2 — relance = task-103 (corriger le dataset d'abord).

**Protections en place depuis 2026-07-13** :

- **sshd blindé** : drop-in `/etc/systemd/system/ssh{,@}.service.d/10-sshd-protection.conf` (OOMScoreAdjust=-1000, MemoryMin=256M, CPUWeight=1000) — plus jamais de perte d'accès sous pression mémoire. Source : `ai-servers/spark-fixes-20260712/`.
- **Preflight FT** : `~/ai-spark/finetune-preflight.sh` appelé par `sparkctl finetune start` (sparkctl:67, backup `sparkctl.bak-20260713-preflight`) — refuse le FT si MemAvailable < 60 Go ; `--auto-stop` arrête whisper/embeddings/reranker/docling/translation (voxcpm conservé).
- **Watchdog Mac** : `ai-servers/spark-fixes-20260712/wait-and-apply.sh` (réutilisable pour un futur incident).

**⚠️ Gotcha boot** : au démarrage du Spark, `spark-whisper` et `spark-translation` initialisent CUDA trop tôt → `cudaErrorNotPermitted`, inférence HTTP 500 alors que le container est « healthy » (le healthcheck ne fait pas d'inférence réelle). **Fix : `docker restart spark-whisper spark-translation` après chaque boot.** Toujours valider par une inférence réelle, pas juste `/health`.

### Fine-tuning terminés (2026-07-10) / file GPU

- **Qwen3.5-4B LoRA `intellisoins-tasks`** : ✅ TERMINÉ 2026-07-10 20:08 (exit 0) — 986 steps, 3h48, eval_loss 1.413, eval_mean_token_accuracy 0.6972, bf16 LoRA r=16 sur base `Qwen/Qwen3.5-4B` (8,8 Go cache HF). Dataset `dgx-fable5-mix` (7888 ex/30M tok : imessage-style 6295, email-style 639, fable5-claude 948, pubmed-opus48 431, fable5-opencode 195, chrome-nav 118). Merge CPU bf16 → `~/finetune-workspace/qwen35-4b-intellisoins-merged` → rewrap VL `qwen35-4b-intellisoins-vl` (8,8 Go, layout Qwen3.5-VL pour vLLM, script `rewrap_qwen35_4b.py`). **Servi maintenant** via `qwen35-4b-finetunev1` :8000 (profil `llm`). Frameworks : TRL 0.24.0, Transformers 5.5.0, PyTorch 2.10.0 (NGC 25.11), image `spark-finetune:trained`. Adapters dans `~/finetune-workspace/adapters-qwen35-4b-tasks/` (checkpoints 850/900/950/986 + final).
...
- **Whisper FT v4** (à migrer) : v3 (380 paires, LoRA) entraîné sur Mac MLX (2026-07-08). La prochaine itération (v4) doit se faire sur le Spark dans le container `spark-finetune` (CUDA, TRL/Transformers/PEFT déjà en place) — sync des wav `~/services/whisper-finetune/recording/` (Mac) → `~/finetune-workspace/whisper/` (Spark) + DB `whisper_dictations` (vérité terrain). Le serving est déjà sur Spark (`spark-whisper` :2022, modèle mergé HF). C'est le seul pipeline de FT restant à centraliser sur le Spark.

`sparkctl finetune start` = ouvre le container NGC PyTorch (workspace persistant) ; `finetune stop` = `docker compose --profile llm down` (arrête `qwen35-4b-finetunev1` :8000 — profil `llm` réintroduit 2026-07-10 avec Qwen3.5-4B FT, renommé 2026-07-11). Historique rollbacks compose : `docker-compose.yml.bak-20260711-spark-full` (renommage + ctx 262144 + sparklan), `docker-compose.yml.bak-20260710-removegemma426b` (retrait Gemma 4 26B), `docker-compose.yml.bak-20260710-qwen35-4b-formation` (ajout Qwen3.5-4B FT).

## ⛔ HISTORIQUE — LLM Gemma 4 26B-A4B FP8 (déployé 2026-07-09, RETIRÉ 2026-07-10) + qwen36-fable5

> **Gemma 4 26B-A4B FP8 supprimé 2026-07-10** (pas assez performant au goût de Michael). Le LLM actuel est **Qwen3.5-4B FT v1 intellisoins** (profil `llm`, `qwen35-4b-finetunev1` :8000, ctx 262144) — voir table services ci-dessus. Cette section est conservée pour les rollbacks (cache HF + `.bak`) et les recettes FP8/MTP réutilisables.
...
- **Profil `llm`** : `qwen35-4b-finetunev1` :8000 (Qwen3.5-4B FT v1, ctx 262144, auth API key). **sparklan (10.0.1.1:8000, ~4 ms) ajouté 2026-07-11** pour Hammerspoon (⌥q/⌥⌘O/⌥⌘ç/⌥4) — publié sur mesh 10.0.0.5:8000 + sparklan 10.0.1.1:8000 + localhost. sparklan publie maintenant : translation :6060, whisper :2022, voxcpm-tts :8026 **et :8000** ; embeddings :8084, reranker :8085, docling :5010 restent **mesh-only** (10.0.0.5, ~82 ms via hub VPS). Vérifié 2026-07-11.
...
| `bge-reranker-v2-m3`       | `hosted_vllm/BAAI/bge-reranker-v2-m3` | `http://10.0.0.5:8085/v1` (repointé Spark 2026-07-12, ex Infinity Mac) |
| `spark-voxcpm-v8`          | `openai/spark-voxcpm-v8`              | `http://127.0.0.1:8884/v1`                                             |

Alias `qwen3-embedding` (Mac :8084) **retiré 2026-07-12** (serveur Mac désactivé) : utiliser `spark-embeddings` (voyage-4-nano **2048D, PAS de matryoshka** — le paramètre `dimensions` est rejeté par vLLM). Les sidecars litellm-pgvector :8093/:8100 sont migrés 2048D (tables `embeddings` re-embedées).

Traduction : NLLB dédié = REST direct `:6060` (spark-translation), pas de LiteLLM Mac.

LLM : `qwen3.5-4b-finetunev1` servi sur Spark `:8000` (container `qwen35-4b-finetunev1`, auth API key `SPARK_VLLM_API_KEY`, ctx 262144, sparklan 10.0.1.1 + mesh 10.0.0.5) — renommé 2026-07-11 (ex `qwen3.5-4b-intellisoins`, alias legacy conservé à vLLM pour la transition). Consommé par Hammerspoon (⌥q/⌥⌘O/⌥⌘ç/⌥4), LiteLLM **VPS** (alias `formation-tutor`, api_base `http://10.0.0.5:8000/v1`, fonctionne via alias legacy) **ET LiteLLM Mac** (`model_name: qwen3.5-4b-finetunev1` dans `litellm-proxy/config.yaml` ; exposé dans `opencode.jsonc` comme `litellm/qwen3.5-4b-finetunev1`, ctx 262144, tool_call true). `SPARK_VLLM_API_KEY` archivée Proton Pass (vault Personal) + miroir `~/.hammerspoon/.env`.

TTS : `spark-voxcpm-v8` via bridge Mac :8884 (→ Spark :8026 Nano-vLLM). Alias `michael-v6-mlx-8bit` (bridge Mac :8883 → :8025 MLX local) **retiré 2026-07-11** (serveur Mac voxcpm désactivé, pas de fallback local).
...
- **Stack Mac décommissionnée 2026-07-11, complétée 2026-07-12 (choix Michael : modèles 100 % Spark)** : toute l'inférence Hammerspoon est 100% Spark — Qwen3.5-4B FT v1 :8000 (⌥q/⌥⌘O/⌥⌘ç/⌥4, ex-Gemma 4 omni local), whisper FT :2022, voxcpm2 v8 :8026 via bridge :8884, NLLB :6060. Les serveurs Mac de modèles (`mlx-vlm-omni` :8089, `whisper-stt` :2022, `voxcpm-tts` :8025, `voxcpm-openai-bridge` :8883, oMLX :8211, **et depuis 2026-07-12 : `embedding` :8084, `reranker` :8085, `gliner` :8091, `docling` :5010**) sont **désactivés** (autostart false, keepalive false) — rechargables manuellement (`./aictl start <name>`) pour un besoin local ponctuel (ex. scripts d'indexation 1024D des bases medical/emails/finances, NER kg_pipeline). **Pas de fallback silencieux** : si Spark down, les raccourcis affichent une alerte explicite (« Spark injoignable — sparkctl status »). Seul fallback conservé : sparklan → mesh (même service Spark, autre chemin réseau). oMLX est dé-pinné (modèle gardé sur disque, rechargeable à la demande).
- **claude_usage embeddings migrés 2026-07-12** : `sessions.embedding` = **halfvec(2048) voyage-4-nano Spark** (ex Qwen3 1024D Mac ; HNSW `vector` plafonne à 2000 dims → halfvec). Pipeline `claude_code_to_training.py` + `ingest_opencode_sessions.py` + backfill appellent `http://10.0.0.5:8084/v1/embeddings` en REST direct (forward-only non-bloquant). Texte brut sans préfixes voyage (cohérence requête↔document).
