...
| spark-whisper     | 2022 | `core` | whisper-large-v3-turbo **fine-tuné voix Michael v3 mergé HF** (FastAPI+CUDA fp16, build `~/ai-spark/whisper/`, modèle `~/ai-spark/whisper/model/`)                  | `/v1/audio/transcriptions` OpenAI (json/text), ~0,36 s/clip (2026-07-10) ; le Mac :2022 (MLX) reste primaire Hammerspoon — le Spark sert frontends distants/agent vocal futur                 |

### Fine-tuning en cours / file GPU (2026-07-10)

- **VoxCPM v8** (voix Michael, dataset complet 225 wav du rescue) : container `voxcpm-v8-finetune`, log `~/finetune-workspace/voxcpm-v8/train-v8-spark.log` — détails rule `apple_all/voxcpm.md`.
- **Qwen3.5-4B LoRA** (`adapters-qwen35-4b-tasks`) : mis en file par `~/finetune-workspace/run-qwen35-4b-after-voxcpm.sh` (nohup sur l'hôte) — attend la fin de VoxCPM, stoppe son container, lance `train_qlora.py` (bf16 LoRA r=16, FT_NO_BNB=1, max_seq 8192, image `spark-finetune:trained`) sur le mix `dgx-fable5-mix` (7888 ex/30M tok : imessage-style 6295, email-style 639, fable5-claude 948, pubmed-opus48 431, fable5-opencode 195, chrome-nav 118). Log : `~/finetune-workspace/qwen35-4b-train.log`. Base `Qwen/Qwen3.5-4B` déjà dans le cache HF (8,8 Go). Après : servir via vLLM (LoRA à chaud ou merge) + lanes calendrier/rappels à générer (phase 2).
