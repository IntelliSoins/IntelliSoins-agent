...
| **G** | **arozanov v2**                      | `MixedQuantKVCache(k_bits=8, v_bits=4)`                      | Mixed-precision, custom fork of mlx-lm                |
...
| **[arozanov/mlx-lm](https://github.com/arozanov/mlx-lm)**                                                    | Mixed-Precision KV cache (fork of mlx-lm)                                      | `MixedQuantKVCache(k_bits, v_bits)` / `VOnlyTurboQuantCache`                |
...
### Quantization de K (Keys) vs V (Values) : Constat Clé

- **Quantization de K** : dégrade fortement le greedy decode en dessous de 4 bits. Les Keys déterminant les poids d'attention, le Softmax y est extrêmement sensible.
- **Quantization de V** : très robuste jusqu'à 3 bits, car l'interpolation pondérée du produit avec les valeurs est résiliente aux petites perturbations.
- **Recommandation** : privilégier la quantization mixte avec **K en 8-bit et V en 4-bit** (ou K=8, V=3 dans la version asymétrique de helgklaizar).
...
## Path G: arozanov v2 Mixed-Precision (mlx-lm fork)

Repo **[arozanov/mlx-lm](https://github.com/arozanov/mlx-lm)** (fork).
Utilise `MixedQuantKVCache` pour implémenter une quantization à précision mixte au niveau du cache KV.

### Installation

```bash
git clone https://github.com/arozanov/mlx-lm.git
cd mlx-lm
pip install -e .
```

### Usage Python (MixedQuantKVCache)

```python
from mlx_lm import load
from mlx_lm.models.cache import MixedQuantKVCache

# Chargement du modèle avec cache à précision mixte : K en 8-bit et V en 4-bit
model, tokenizer = load(
    "mlx-community/Meta-Llama-3-8B-Instruct-4bit",
    cache_class=MixedQuantKVCache,
    cache_kwargs={"k_bits": 8, "v_bits": 4}
)
```

### Option V-Only TurboQuant Cache (compatible stock mlx-lm)

Alternative de arozanov compatible avec la version stock (officielle) de `mlx-lm` (sans fork).

- **Configuration** : K (Keys) est conservé en fp16 (aucune dégradation du softmax) et V (Values) est quantisé en PolarQuant 3-bit.
- **Bénéfice** : Zéro perte sur la précision d'attention tout en réduisant de ~50% la taille globale du cache KV.

```python
from mlx_lm import load
from mlx_lm.models.cache import VOnlyTurboQuantCache

# Compatible stock mlx-lm : K conservé en fp16, V compressé en 3-bit PolarQuant
model, tokenizer = load(
    "mlx-community/Meta-Llama-3-8B-Instruct-4bit",
    cache_class=VOnlyTurboQuantCache,
    cache_kwargs={"v_bits": 3}
)
```
...
- **Butterfly-pulled-out optimization** : 4.5x speedup sur V-attention en factorisant la WHT (Walsh-Hadamard Transform).
- **SIMD-group reductions** : 1.85x speedup via `simd_sum` au lieu d'une réduction en arbre.
- **Flash-attention scaffold** : fused SDPA (Scaled Dot-Product Attention) direct sur cache compressé.
