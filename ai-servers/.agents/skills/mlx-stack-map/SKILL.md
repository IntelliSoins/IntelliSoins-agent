...
| ollama (runner MLX) | ollama/ollama              | v0.24.0 (2026-05-14 ; runner MLX inchangé)            | runner MLX natif safetensors                      | `~/.claude/rules/ollama-mlx.md`                                                |

> ⚠️ Fork `Blaizzy/vllm-omni` = MORT (dernier push 2026-02-01, aucun tag) — utiliser `vllm-project/vllm-omni`.

> Les 4 derniers (omlx, mlx-omni-server, mlx-openai-server, ollama-mlx) ont été migrés du plugin `intellisoins-mlx` vers ces rules le 2026-05-29 ; leurs skills sources sont en staging `~/.claude/plugins-staging/mlx-servers/` (désactivés, réversibles).
...
Skills `intellisoins-mlx:` — `mlx-fine-tuning`, `mlx-vlm-fine-tuning`, `mlx-tune` (ARahim3/mlx-tune **v0.5.0**, 2026-05-19), `ollama-mlx-fine-tuning`, `gemma3-finetune`, `gemma4-mlx`, `medgemma-finetune`, `medgemma-prompting`, `nemotron-finetune`, `qwen3-finetuning-tool-calling`, `qwen35-vision-finetuning`, `gliner-ner`, `bert-relation-extraction`.
