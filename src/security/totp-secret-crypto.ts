...
function decryptTotpSecretWithKey(payload: string, key: Buffer): string | null {
...
export function encryptTotpSecret(secret: string, env?: NodeJS.ProcessEnv): string {
  const key = resolveControlUiTotpEncryptionKey(env);
...
/** Decrypt a stored TOTP secret payload using the persisted credentials key. */
export function decryptTotpSecret(payload: string, env?: NodeJS.ProcessEnv): string | null {
  return decryptTotpSecretWithKey(payload, resolveControlUiTotpEncryptionKey(env));
}

/** Decrypt a legacy path-derived TOTP payload for migration only. */
export function decryptLegacyTotpSecret(payload: string, env?: NodeJS.ProcessEnv): string | null {
  return decryptTotpSecretWithKey(payload, deriveLegacyControlUiTotpEncryptionKey(env));
}
