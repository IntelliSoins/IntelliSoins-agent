...
function renderPendingApprovalNote(kind: PendingDeviceApprovalKind) {
  switch (kind) {
    case "scope-upgrade":
      return t("nodes.devices.approvalNotes.scopeUpgrade");
    case "role-upgrade":
      return t("nodes.devices.approvalNotes.roleUpgrade");
    case "re-approval":
      return t("nodes.devices.approvalNotes.reApproval");
    case "new-pairing":
      return t("nodes.devices.approvalNotes.newPairing");
  }
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">
          ...
          ${t("nodes.devices.requested", {
            time: `${age}${repair}`,
          })}
        </div>
        <div class="muted" style="margin-top: 6px;">
          ${t("nodes.devices.requestedAccess", { access: formatAccessSummary(approval.requested) })}
        </div>
        ${approval.approved
          ? html`
              <div class="muted" style="margin-top: 6px;">
...
function renderTokenRow(deviceId: string, token: DeviceTokenSummary, props: NodesProps) {
  ...
  const scopes = t("nodes.devices.scopes", { scopes: formatList(token.scopes) });
      ...
      <div class="list-main">
        ...
        <div class="list-sub">
          ...
          ${bindingValue === "__default__"
            ...
            : t("nodes.binding.overrideBinding", { value: agent.binding ?? "" })}
        </div>
      </div>
      <div class="list-meta">
        <label class="field">
...
function renderNode(node: Record<string, unknown>) {
  ...
  const title =
    ...
    (typeof node.nodeId === "string" ? node.nodeId : t("nodes.nodeCard.unknown"));
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip"
            >${paired ? t("nodes.nodeCard.paired") : t("nodes.nodeCard.unpaired")}</span
          >
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t("nodes.nodeCard.connected") : t("nodes.nodeCard.offline")}
          </span>
