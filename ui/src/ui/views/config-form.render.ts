...
// Section metadata resolved at render time for locale support.
export function getSectionMeta(key: string): { label: string; description: string } {
  const labelKey = `configForm.sections.${key}.label`;
  const descriptionKey = `configForm.sections.${key}.description`;
  const label = t(labelKey);
  const description = t(descriptionKey);
  if (label !== labelKey) {
    return { label, description };
  }
  return { label: humanize(key), description: "" };
}
    ...
    return html`
      <div class="config-empty">
        ...
        <div class="config-empty__text">
          ${searchQuery
            ? t("configForm.noMatch", { query: searchQuery })
